...
const MyVideos = () => {
  ...
  const getOwnVideo = () => {
    axios
      .get('https://you-tube-clone-o3nx.vercel.app/video/own-video', {
        headers: {
  ...
  const handleDelete = (videoId) => {
    if (window.confirm('Are you sure you want to delete this video?')) {
      axios
        .delete(`https://you-tube-clone-o3nx.vercel.app/video/${videoId}`, {
          headers: {
            Authorization: 'Bearer ' + localStorage.getItem('token'),
          },
        })
  ...
  const handleEdit = (video) => {
    ...
    if (newTitle && newDescription && newCategory && newTags) {
      axios
        .put(
          `https://you-tube-clone-o3nx.vercel.app/video/${video._id}`,
          {
            title: newTitle,
