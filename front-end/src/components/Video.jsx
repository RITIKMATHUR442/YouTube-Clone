...
const Video = () => {
  ...
  const incrementView = async (videoId) => {
    try {
      const response = await axios.put(`https://you-tube-clone-o3nx.vercel.app/video/views/${videoId}`);
  ...
  const fetchComments = async () => {
    try {
      ...
      const response = await axios.get(`https://you-tube-clone-o3nx.vercel.app/comment/${videoId}`);
      if (response.status === 200) {
      ...
      if (editingIndex !== null) {
        ...
        try {
          ...
          const response = await axios.put(
            `https://you-tube-clone-o3nx.vercel.app/comment/${editingCommentId}`,
            { commentText: comment },
        ...
        try {
          ...
          const response = await axios.post(
            `https://you-tube-clone-o3nx.vercel.app/comment/new-comment/${videoId}`,
            { commentText: comment },
            { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
          );
  ...
  const handleDeleteComment = async (index, commentId) => {
    try {
      const response = await axios.delete(
        `https://you-tube-clone-o3nx.vercel.app/comment/${commentId}`,
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
  ...
  const handleLike = async () => {
    try {
      const videoId = location.state.video._id;
      const response = await axios.put(
        `https://you-tube-clone-o3nx.vercel.app/video/like/${videoId}`,
        {},
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
  ...
  const handleDislike = async () => {
    try {
      const videoId = location.state.video._id;
      const response = await axios.put(
        `https://you-tube-clone-o3nx.vercel.app/video/dislike/${videoId}`,
        {},
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
  ...
  const handleSubscribe = async () => {
    try {
      const response = await axios.post(
        `https://you-tube-clone-o3nx.vercel.app/user/subscribe/${location.state.user_id._id}`,
        {},
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
  ...
  const handleUnsubscribe = async () => {
    try {
      const response = await axios.post(
        `https://you-tube-clone-o3nx.vercel.app/user/unsubscribe/${location.state.user_id._id}`,
        {},
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
